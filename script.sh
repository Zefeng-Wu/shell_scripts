...
# 2. 得到reads count进行差异表达分析
# gffread Lotusjaponicus_MG20_v3.0_annotations.gff3 -o Lotusjaponicus_fc.gff3 -O #不行，还是在转录本水平定量-g 
gffread Lotusjaponicus_MG20_v3.0_annotations.gff3 -T -o Lotusjaponicus.gtf
mkdir 2feature_count
for m in $(ls 1hisat2_out); do echo $m; featureCounts -a ~/Genome.db/gff3/Lotusjaponicus.gtf -o 2feature_count/${m%.sam}.fc  -p -T 12 1hisat2_out/$m; done
...
# 4. Stringtie
mkdir 4stringtie
for m in $(ls 3sorted_bam/*.bam); do echo $m; stringtie $m  -G ~/Genome.db/gff3/Lotusjaponicus.gtf  -e -B -A 4stringtie/$(basename ${m%.sorted.bam}).genes.tsv -o 4stringtie/$(basename ${m%.sorted.bam}).gff -p 10; done
...
### 2.6 用基因树推断物种树(先用raxml构建多个基因树+stral流程等推断物种树)
#2.6.1 修改多序列比对header
